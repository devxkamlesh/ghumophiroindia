...
async function ToursContent({ searchParams }: { searchParams: { destination?: string; duration?: string; category?: string; sort?: string } }) {
  // Fetch real tours from database
  let result
  try {
    result = await toursService.getAll({
  ...
  if (searchParams.duration) {
    const [min, max] = searchParams.duration.split('-').map(d => d.replace('+', ''))
  ...
  if (searchParams.sort) {
    switch (searchParams.sort) {
      ...
      case 'duration':
        ...
        break
      case 'popular':
        tours = [...tours].sort((a, b) => b.reviewCount - a.reviewCount)
        break
    }
  }
  ...
  if (tours.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <Link href="/tours" className="inline-block bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors">
          View All Tours
        </Link>
      </div>
    )

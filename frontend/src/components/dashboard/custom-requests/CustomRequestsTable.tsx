        ...
        <tbody className="divide-y divide-gray-200">
          {requests.map((request) => (
            <tr key={request.id} className="hover:bg-gray-50">
                ...
                {request.destinations && request.destinations.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {request.destinations.map((dest) => (
                      <span key={dest} className="px-2 py-1 bg-primary-50 text-primary-700 text-xs rounded">{dest}</span>
                    ))}
                  </div>
                ) : (
              ...
              <td className="px-6 py-4">
                {request.budget ? (
                  <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded capitalize">{request.budget}</span>
                ) : (
                  <span className="text-gray-400">-</span>
                )}
              </td>

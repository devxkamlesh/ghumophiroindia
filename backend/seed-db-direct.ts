...
// Run with: npx tsx seed-db-direct.ts

import db from './src/core/database'
import { users, tours } from './src/core/database/schema'
...
async function seedDatabase() {
  ...
  try {
    ...
    console.log(`   - Tours created: ${createdCount}`)
    console.log(`   - Tours skipped: ${skippedCount}`)
    console.log('\n🔐 Admin credentials:')
    console.log(`   Email: ${adminUser.email}`)
    console.log(`   Password: ${adminUser.password}`)
    console.log('\n🌐 Test the API:')
    console.log(`   - http://187.127.151.137/api/v1/tours`)
    console.log(`   - http://187.127.151.137/api/v1/tours/featured`)
